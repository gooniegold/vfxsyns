...
/* ── Bio links only (socials moved to HomeTab) ── */
const BIO_LINKS = [
  {
    label: "Steam",
    handle: "heist / synfx",
  ...
  return (
    <motion.a
      ...
      onMouseEnter={(e) => {
        ...
        el.style.background = `rgba(${link.accent}, 0.07)`;
        el.style.borderColor = `rgba(${link.accent}, 0.28)`;
        el.style.transform = "translateX(2px)";
        el.style.boxShadow = `0 0 20px rgba(${link.accent}, 0.06)`;
      }}
      onMouseLeave={(e) => {
        const el = e.currentTarget as HTMLElement;
    ...
    >
      ...
      <div
        ...
        style={{
          background: `rgba(${link.accent}, 0.12)`,
          color: `rgba(${link.accent}, 0.9)`,
        }}
      >
        {link.icon}
      </div>
      ...
      <div className="min-w-0 flex-1">
        ...
        <p
          className="mt-[4px] text-[10px] leading-none"
      ...
      {/* Arrow */}
      <svg
        className="h-3 w-3 shrink-0 opacity-0 transition-all duration-200 group-hover:opacity-100 group-hover:translate-x-0.5"
    ...
    >
      ...
      <div className="space-y-1.5">
        {BIO_LINKS.map((link, i) => (
          <LinkRow key={link.label} link={link} delay={i * 0.05} />
        ))}
      </div>
    </motion.div>
  );
