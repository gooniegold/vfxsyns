...
function LinkRow({
  ...
  link: { label: string; handle: string; url: string; accent: string; icon: React.ReactNode };
  delay: number;
}) {
  return (
    <motion.a
      ...
      onMouseEnter={(e) => {
        ...
        el.style.borderColor = `rgba(${link.accent}, 0.25)`;
        el.style.boxShadow = `0 0 16px rgba(${link.accent}, 0.07)`;
      }}
      onMouseLeave={(e) => {
        const el = e.currentTarget;
    ...
    >
      ...
      {/* Arrow */}
      <svg
        className="h-3 w-3 shrink-0 transition-all duration-200 group-hover:translate-x-0.5"
...
export function LinksTab() {
  return (
    ...
    >
      {/* Bio Links */}
      <div>
        <SectionLabel text="Bio Links" />
        <div className="space-y-1.5">
          {BIO_LINKS.map((link, i) => (
            <LinkRow key={link.label} link={link} delay={i * 0.04} />
          ))}
        </div>
      </div>
      ...
      {/* Socials */}
      <div>
        <SectionLabel text="Socials" />
        <div className="space-y-1.5">
          {SOCIAL_LINKS.map((link, i) => (
            <LinkRow key={link.label} link={link} delay={BIO_LINKS.length * 0.04 + i * 0.04} />
          ))}
        </div>
      </div>
    </motion.div>
